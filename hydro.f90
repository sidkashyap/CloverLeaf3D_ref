  ...
  DO
    ...
    IF(time+g_small.GT.end_time.OR.step.GE.end_step) THEN
      ...
      IF ( profiler_on ) THEN
        ...
        CALL clover_allgather_count(profiler%advec_mom_flop,totals)
        profiler%advec_mom_flop=totals(loc(1))
        CALL clover_allgather_count(profiler%advec_cell_flop,totals)
        profiler%advec_cell_flop=totals(loc(1))
        CALL clover_allgather_count(profiler%advec_mem,totals)
